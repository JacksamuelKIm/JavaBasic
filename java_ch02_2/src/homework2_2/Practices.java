...
//		operationsSwitch(); // 12-2번
	...
	public void digitComparison() {
		...
		if (tens == ones) {
			...
			System.out.println("No! 10의 자리와 1의 자리가 같지 않습니다"); // /와 % 연산자가 괜히 만들어진게 아니겠지. 수학임
		}
	...
	public void game369() {
		...
		// 그런데 (0%3)도 0이지..... 10에서 박수쳐가 나오면 안됨 3의 배수라는건 당연히 0은 아닐때 얘기
		...
		while (num < 1 || num > 99) {
			System.out.println("범위 밖의 수야. 다시 입력해");
			System.out.print("1~99 사이의 정수를 입력하시오 >> ");
			 num = scan6.nextInt();
		}
		
		int x = num / 10; // 십의 자리의 수
		int y = num % 10; // 일의 자리의 수

		if ((x != 0) && (y != 0) && (x % 3 == 0) && (y % 3 == 0)) { // 두자리 이면서, 두 자리 모두 369중 하나에 해당
			System.out.println("박수짝짝");
		} else if ((x == 0) &&  (x % 3 != 0) && (y % 3 == 0)) { // 십의 자리는 0이고, 일의 자리 3 6 9 세 개만 해당
			System.out.println("박수짝");  // while로 0이 못들오어게 막았으니 y!=0 이라는 조건을 굳이 해줄 필요가 없지
